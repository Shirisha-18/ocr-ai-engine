...
# Function to auto-generate labeled training examples
def generate_training_example(text):
    entities = []

    # Patent numbers
    m = re.search(r"(?:Patent No\.?|No\.|Appl\. No\.|Serial No\.):?\s*([\d,]+)", text, re.IGNORECASE)
    if m:
        entities.append((m.start(1), m.end(1), "PATENT_NUMBER"))

    # Dates
    for m in re.finditer(r"(?:Patented|Issued|Specification dated|Mon\.?\s+\d{1,2},\s+\d{4}|Application filed|Filed):?\s*(Mon\.?\s+\d{1,2},\s+\d{4})?", text):
        label = "PATENT_DATE" if "Patent" in m.group() or "Specification" in m.group() else "APPLICATION_DATE"
        entities.append((m.start(), m.end(), label))

    # Inventor / Applicant / Assignee
    for match in re.finditer(r"(Inventor|Applicant|Assignee|Assignors?)[:\.]?\s*([A-Za-z.,\s]+)", text):
